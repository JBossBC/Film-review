...
# query movies by title and kind ,support  fuzzy query
def queryMoviesByTitle(params):
    return movieDF.filter(movieDF.title.like('{}%'.format(params["title"]))).filter(movieDF.genres.like('%{}%'.format(params["kind"]))).toJSON().collect()
